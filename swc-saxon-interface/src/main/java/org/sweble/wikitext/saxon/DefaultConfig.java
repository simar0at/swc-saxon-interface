...
/**
 * Programatically generate a default configuration for a Wiki. At least the namespaces will almost always be different.
 */
public abstract class DefaultConfig {
	
	protected WikiConfigImpl c = null;
	protected ParserConfigImpl pc = null;
	
	protected static WikiConfigImpl generate(DefaultConfig d) {
		d.configureParser();
		d.configureCompiler();
		d.configureWiki();
		return d.c;
	...
	protected DefaultConfig() {
		c = new WikiConfigImpl();
		pc = c.getParserConfig();
	}
	
	protected DefaultConfig(WikiConfigImpl c) {
		this.c = c;
		pc = c.getParserConfig();
	}
	
	protected void configureWiki()
	{
	...
	protected abstract void addNamespaces();
	
	protected void addInterwikis()
	{
		...
		c.addInterwiki(new InterwikiImpl(
				"ckb", 
				"http://ckb.wikipedia.org/wiki/$1",
				true,
				false));

		c.addInterwiki(new InterwikiImpl(
				"koi", 
				"http://koi.wikipedia.org/wiki/$1",
				true,
				false));

		c.addInterwiki(new InterwikiImpl(
				"frr", 
				"http://frr.wikipedia.org/wiki/$1",
				true,
				false));

		c.addInterwiki(new InterwikiImpl(
				"mrj", 
				"http://mrj.wikipedia.org/wiki/$1",
				true,
				false));

		c.addInterwiki(new InterwikiImpl(
				"pnb", 
				"http://pnb.wikipedia.org/wiki/$1",
				true,
				false));
	}
	
	protected void addI18nAlises()
	{
		/*
